...
window.addEventListener('DOMContentLoaded', function(){
	...
	// Inject build status to pull requests
	var comments = document.querySelectorAll('.page-pullrequest .discussion-timeline .avatar-bubble');
	if(comments.length > 0){
		...
		if(last){
			...
			var image = createStatusImage(status);
			if(image){
				var place = document.querySelector('.pull-head .number');
	 ...
	 */
	function createStatusImage(status){
